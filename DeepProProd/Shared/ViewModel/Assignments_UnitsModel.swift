       ...
       // cell.unitDetailLabel.text = String(format:"Status: %d| Attempts: %d| Highest Score:%d",(unitList[indexPath.row].base?.unit_status)!,(unitList[indexPath.row].base?.attempts)!,(unitList[indexPath.row].base?.highest_score)!);
        ...
        if let unitStatus =  UnitStatus(rawValue:(unit.base?.status)!) {
           // cell.assignmentStatus.text = (unit[indexPath.row].base?.unit_status)!
            switch unitStatus {
            case .assigned:
                cell.detailView.backgroundColor = UIColor.hexStringToUIColor(hex: ThemeManager.sharedInstance.units_Assigned!)
                break
            case .submitted :
                cell.detailView.backgroundColor =  UIColor.hexStringToUIColor(hex: ThemeManager.sharedInstance.units_Submitted!)
                break
            case .reviewed:
                cell.detailView.backgroundColor =  UIColor.hexStringToUIColor(hex: ThemeManager.sharedInstance.units_Reviewed!)
                break
            }
        }
